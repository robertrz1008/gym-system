...
export const createPaymetRequest = async (req: Request, res: Response) => { 
    ...
    try {
        const pgClient = await connectdb.connect()
        const sqlQuery = "INSERT INTO payments_membership(id_client, id_pay_option, pay_date, expiration_date) VALUES($1, $2, $3, $4)"
...
export const expireMembershipRequest = async (req: Request, res: Response) => {
    ...
    try {
        const pgClient = await connectdb.connect()
        await pgClient.query("update clients set id_status = 2 where id = $1", [id_client])
        res.status(501)
        pgClient.release()
